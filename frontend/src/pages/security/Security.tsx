...
import {
  ArrowLeft, ChevronRight, Shield, Key, Smartphone,
  Lock, Clock, Trash2, Eye, EyeOff, Check,
        ...
        {[
          ...
          <div key={f.key} style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input
                type={(show as any)[f.showKey] ? 'text' : 'password'}
...
const PhoneBinding = ({ onBack, currentPhone }: any) => {
  ...
  const [loading, setLoading] = useState(false);
  const otpRefs               = useState<any[]>(Array(6).fill(null))[0];
  ...
  const handleOtpChange = (i: number, val: string, refArr: any[]) => {
    const v = val.replace(/\D/g,'').slice(-1);
    const next = [...otp]; next[i] = v; setOtp(next);
    if (v && i < 5) refArr[i+1]?.focus();
    ...
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 20px',
        ...
        <span style={{ fontWeight: 700, fontSize: 17 }}>
          {currentPhone ? 'Change Phone' : 'Bind Phone Number'}
        </span>
      </div>
      ...
      <div style={{ padding: 24 }}>
        {step === 'input' ? (
          <>
            <p style={{ color: 'var(--color-muted)', fontSize: 14,
              ...
              {currentPhone
                ...
                : 'Bind your phone number for extra security and easy login.'}
            </p>
          ...
          <>
            ...
            <p style={{ fontWeight: 700, marginBottom: 28 }}>+{phone}</p>
            ...
            <div style={{ textAlign: 'center' }}>
              <button onClick={async () => {
                ...
                background: 'none', border: 'none', cursor: resendSec > 0 ? 'not-allowed' : 'pointer',
        ...
        {loading ? (
          ...
          <div key={i} style={{ padding: '14px', borderRadius: 12, marginBottom: 10,
            ...
            <div style={{ fontSize: 12, color: 'var(--color-muted)', lineHeight: 1.6 }}>
              ...
              {h.device_type && <span> • {h.device_type}</span>}
            </div>
            {h.user_agent && (
...
const AntiPhishing = ({ onBack, current }: any) => {
  ...
  return (
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: 20 }}>
        <div style={{ padding: 16, borderRadius: 12, background: '#f0b90b15',
                      border: '1px solid #f0b90b30', marginBottom: 20 }}>
          <p style={{ color: 'var(--color-text)', fontSize: 13, lineHeight: 1.6, margin: 0 }}>
            🛡️ Set a unique code that will appear in all VDExchange emails.
...
export default function Security() {
  ...
  useEffect(() => {
    fetchProfile();
    // Auto-open phone tab if navigated with state
  ...
  const secScore = [
    profile?.phone,
    twoFAEnabled,
    profile?.anti_phish_code,
    profile?.kyc_level > 0,
  ].filter(Boolean).length;

  const scoreLabel = secScore <= 1 ? 'Low ⚠️'
    : secScore === 2 ? 'Medium 🔒'
    : secScore === 3 ? 'Good 🔒'
    : 'High 🔒';
  const scoreColor = secScore <= 1 ? 'var(--color-danger)'
    : secScore === 2 ? 'var(--color-warning)'
    : 'var(--color-success)';
  ...
  const items = [
    ...
    },
    {
      ...
      sub: profile?.phone
        ? `✅ ${profile.phone.slice(0,3)}****${profile.phone.slice(-3)}`
        : '⚠️ Not bound — recommended',
    ...
    },
    {
      ...
      sub: twoFAEnabled ? '✅ 2FA Enabled' : '⚠️ Not enabled — recommended',
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ background: 'var(--color-surface)', borderRadius: 16,
          ...
          {items.map(({ icon: Icon, label, sub, subColor, action, toggle,
                        toggled, onToggle, color, danger }: any, i) => (

...
const OrderBook = ({ bids, asks, currentPrice, isUp, onPriceClick }: any) => {
  const allQtys = [...asks.map((a: any) => parseFloat(a.qty)||0), ...bids.map((b: any) => parseFloat(b.qty)||0)];
        ...
        return (
          <div key={i} onClick={() => onPriceClick(parseFloat(ask.price||0).toFixed(2))}
            ...
            <span style={{ color:'var(--color-text)', position:'relative', zIndex:1 }}>
              {parseFloat(ask.qty||0).toFixed(4)}</span>
          </div>
        );
      })}
      ...
      {bids.slice(0,8).map((bid: any, i: number) => {
        ...
        return (
          <div key={i} onClick={() => onPriceClick(parseFloat(bid.price||0).toFixed(2))}
            ...
            <span style={{ color:'var(--color-text)', position:'relative', zIndex:1 }}>
              {parseFloat(bid.qty||0).toFixed(4)}</span>
          </div>
        );
      })}
    </div>
...
const RecentTrades = ({ trades }: any) => (
  <div style={{ fontSize:12 }}>
    ...
    {trades.length === 0
      ? <div style={{ textAlign:'center', padding:20, color:'var(--color-muted)' }}>No trades</div>
      : trades.map((t: any, i: number) => (
        <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'3px 8px' }}>
          <span style={{ color: t.side==='buy' ? 'var(--color-success)' : 'var(--color-danger)' }}>
            {parseFloat(t.price||0).toFixed(2)}</span>
...
export default function Trade() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh',
      ...
      <div style={{ background:'var(--color-surface)', flexShrink:0, padding:'10px 12px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, cursor:'pointer' }}
            ...
            <span style={{ fontWeight:700, fontSize:17, color:'var(--color-text)' }}>{baseSym}</span>
      ...
      {/* ===== MIDDLE: Order Book + Form (SPLIT) ===== */}
      <div style={{ display:'flex', flexShrink:0 }}>

        {/* LEFT: Order Book - fixed height */}
        <div style={{ width:'42%', maxHeight:'55vh', overflow:'auto' }}>
          <div style={{ padding:'4px 8px', fontSize:11, color:'var(--color-muted)',
        ...
        {/* RIGHT: Order Form - fixed height, scrollable */}
        <div style={{ flex:1, maxHeight:'55vh', overflow:'auto', padding:'8px 10px' }}>
              ...
              : openOrders.map((o: any) => (
                <div key={o.order_id} style={{ padding:'12px 16px',
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between',
                    ...
                    <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                      <span style={{ padding:'2px 8px', borderRadius:4, fontSize:11, fontWeight:700,
                        ...
                        textTransform:'uppercase' }}>{o.side}</span>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:12 }}>
                    ...
                    <span style={{ color:'var(--color-muted)' }}>
                      Filled: <span style={{ color:'var(--color-text)' }}>
                        0/{parseFloat(o.remaining_qty||0).toFixed(5)}
                      </span>
                    </span>
                  </div>
                </div>
